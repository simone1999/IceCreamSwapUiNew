...
export const ImpermanentLossCalculator = memo(function ImpermanentLossCalculator({
  ...
  const exitAssets = useMemo<Asset[] | undefined>(() => {
    if (assets && isFarm && currencyA && currencyA.chainId in ICE && cakePrice) {
      const cakePriceToUse =
        assets.find((a) => a.currency.equals(ICE[currencyA.chainId as keyof typeof ICE]))?.price ?? cakePrice;
