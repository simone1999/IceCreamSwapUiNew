...
export const useCake = () => {
  ...
  return useContract(((chainId && ICE[chainId]?.address) ?? ICE[ChainId.CORE].address) as Address, erc20ABI)
}
...
export const usePredictionsContract = (address: Address, tokenSymbol: string) => {
  ...
  return useMemo(() => {
    ...
    const getPredContract = tokenSymbol === 'ICE' ? getCakePredictionsContract : getPredictionsV2Contract
...
export function useMasterchefV3() {
  ...
  return useMemo(() => {
    if (SUPPORT_FARMS_V3.includes(chainId)) {
      return getMasterChefV3Contract(signer ?? undefined, chainId)
    }
    return undefined
  }, [signer, chainId])
}
